...
## Responsibilities
- **Task Planning & Delegation**: Break down complex objectives and use `vibe_create_task` to deploy Workers to handle specific sub-tasks. Use `vibe_plan` to outline phases and tasks.
...
- **Consolidation**: Once all tasks are complete, aggregate the results and generate final documentation (e.g., using `vibe_release_summary` as a standard phase tool).

## Workflow
1. Plan and break down the task using `vibe_plan`.
...
6. Finalize with `vibe_release_summary` when the phase is complete.