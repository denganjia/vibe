...
**Milestone Goal:** 让用户通过安装 plugin 启用多模型协作，而不是学习 and 操作独立重型 CLI；把必要 runtime 能力瘦身为 plugin/scripts，并保留 `.vibe` 作为可观察、可恢复的项目工作区。
...
**Goal**: Plugin 启用后可以非破坏性创建项目级 `.vibe` 工作区，并用 `.vibe/Agents` 定义 planner、executor、reviewer、release 等角色 and 模型命令。
**Depends on**: Phase 20
...
**Success Criteria** (what must be TRUE):
  1. scripts 可以 create 包含 goal, context, file scope, constraints, expected output, verification command 和 reviewer requirements 的 task JSON。
...
**Plans**: 1 plans
Plans:
- [x] 22-01-PLAN.md — Implementation of core scripts (task, lock, run, status). (completed 2026-04-23)
